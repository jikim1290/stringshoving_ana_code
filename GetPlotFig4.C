...
void GetPlotFig4(){
 ...
 TFile* fin = new TFile("output_set05_grp000_ana2.root","read");
 ...
 TH2D* hCorr_Fig4[nMult][nPtTrig];
 TH2D* hCorrMixing_Fig4[nMult][nPtTrig];
 ...
 for(int i=0;i<nMult;i++){
	for(int j=0;j<nPtTrig;j++){
		hCorr_Fig4[i][j] = (TH2D*)fin->Get(Form("hCorr_Fig4_%d",i));
		hCorrMixing_Fig4[i][j] = (TH2D*)fin->Get(Form("hCorrMixing_Fig4_%d",i));

		hC[i][j] = (TH2D*)hCorr_Fig4[i][j]->Clone();

		hCorrMixing_Fig4[i][j]->Scale( 1.0/hCorrMixing_Fig4[i][j]->GetMaximum() );
		hC[i][j]->Divide( hCorrMixing_Fig4[i][j] );
 ...
 for(int i=0;i<nMult;i++){
	...
	ycntl[i] = YieldCntl[i][0];
	ystat[i] = YieldStat[i][0];
 }


 TGraphErrors* g = new TGraphErrors(nMult,mcntl,ycntl,mstat,ystat);
 g->SetLineColor(kRed);
 ...
 for(int i=0;i<16;i++){
	c->cd(i+1);
	if( hFPhiZYAM[i][0] ) hFPhiZYAM[i][0]->Draw();
 }
