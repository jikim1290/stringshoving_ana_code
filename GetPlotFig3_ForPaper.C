...
//                      if( j*5 + 2.5 > MultMin[i] && j*5 + 2.5 < MultMax[i] ){
 ...
 ghep_cms->GetXaxis()->SetTitle("#font[12]{p}_{T,trig(assoc)} (GeV/c)");
...
// g[3]->SetFillStyle(1);
 ...
 TLegend* leg2 = new TLegend(0.4,0.56,0.89,0.89);
 leg2->SetLineWidth(0.0);
 leg2->SetFillColorAlpha(0,0);
 ghep_cms->Draw("AP");
 leg2->AddEntry( (TObject*)0, "pp, #sqrt{s}=13 TeV", "" );
 leg2->AddEntry( ghep_cms, "CMS, #font[12]{N}_{ch}>110", "p" );
 leg2->AddEntry( g[0], "PYTHIA8 default, #font[12]{N}_{ch}>90", "f" );
 leg2->AddEntry( g[3], "PYTHIA8 String Shoving, #font[12]{N}_{ch}>90", "f" );
 g[0]->Draw("3C"); g[3]->Draw("3");
 leg2->Draw();

 c->SaveAs("figs_paper/Reprodpt.pdf");
